...
import { ArrowLeft, Search, Clock, UserCheck, UserX, CalendarClock, Edit, Save, X, CheckCircle, Info, Users } from 'lucide-react';
...
interface ManageTodayAttendanceProps {
  onBack: () => void;
}

export default function ManageTodayAttendance({ onBack }: ManageTodayAttendanceProps) {
  const { t } = useLanguage();
  ...
  const fetchTodayAttendance = async () => {
    try {
      ...
      const records = data.map((item: any) => {
        ...
        // In a real system, you'd have a separate punches table
  ...
  const handlePunchesUpdate = async (recordId: string, newPunches: AttendancePunch[]) => {
    ...
    try {
      ...
      // Note: Backend currently supports single check-in/out. 
      // In a full implementation, we would store all punches in a separate table.
  ...
  const handleManualOverride = async (recordId: string, status: 'On Leave' | 'Absent' | null) => {
    const record = attendanceRecords.find(r => r.id === recordId);
    if (!record) return;

    try {
      const apiStatus = status === 'On Leave' ? 'leave' : status === 'Absent' ? 'absent' : 'present';
  ...
  const summaryStats = useMemo(() => {
    ...
    attendanceRecords.forEach(record => {
      if (record.manualOverride) {
        if (record.manualOverride.status === 'On Leave') onLeave++;
        else if (record.manualOverride.status === 'Absent') absent++;
      } else {
    ...
    return { present, late, absent, halfDay, onLeave, total };
  }, [attendanceRecords]);
  ...
  const columns = [
    {
      accessor: 'name',
      header: 'Employee',
      cell: (record: TodayAttendanceRecord) => (
        <AvatarCell
          name={record.name}
          subtitle={record.id}
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div>
            <h1 className="text-3xl">Manage Today's Attendance</h1>
            <Breadcrumbs
              items={[
                { label: 'Dashboard' },
                { label: 'Attendance' },
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-blue-700 mb-2">
                Attendance is automatically captured from face recognition devices. Employees can check in and out multiple times throughout the day.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-blue-700">
                <div>• <strong>First Check-In:</strong> Earliest punch of the day</div>
                <div>• <strong>Last Check-Out:</strong> Latest punch of the day</div>
                <div>• <strong>Work Hours:</strong> Total time excluding breaks</div>
                <div>• <strong>Break Time:</strong> Gaps between check-out and next check-in</div>
